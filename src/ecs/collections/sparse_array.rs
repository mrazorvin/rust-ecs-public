...
macro_rules! bits {
    (@set $self:ident, $request_id:ident) => {
        let new_chunks_size = $request_id / crate::ecs::collections::sparse_array::BITS_SIZE + 1;
        ...
        let bits = unsafe { $self.bits.get_unchecked_mut(($request_id / crate::ecs::collections::sparse_array::BITS_SIZE) as usize) };
